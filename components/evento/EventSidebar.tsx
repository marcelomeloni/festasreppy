...
import { OfficialLot, MarketLot, PlatformFee, EventData } from "@/services/eventsService";
...
// Converte decimal para string percentual legível: 0.07 → "7%", 0.085 → "8,5%"
function formatPercent(decimal: number): string {
  const pct = decimal * 100;
  const rounded = Math.round(pct * 10) / 10; // 1 casa decimal se necessário
...
export function EventSidebar({
  ...
  const fee = {
    percentage: safeNum(platformFee?.percentage), // decimal: 0.07 = 7%
  ...
  const inc = (id: string) => {
    const lot = officialLots.find(l => l.id === id);
  ...
  function resolveFinalPrice(lot: OfficialLot): number {
    ...
    if (base === 0) return 0;
    // Usa feePercentage do próprio lote (calculado por faixa no backend)
    const pct = safeNum(lot.feePercentage);
    return lot.feePayer === "buyer"
      ? base + base * pct + fee.fixed
      : base;
  }

  function handleCheckout() {
    const availableLots = officialLots.filter(l => l.available);
  ...
  const totalQty  = officialLots.reduce((s, l) => s + (l.available ? (qtys[l.id] || 0) : 0), 0);
  const cartLines = officialLots
    .filter(l => l.available && (qtys[l.id] || 0) > 0)
    .map(l => {
      const qty        = qtys[l.id];
      const base       = safeNum(l.price);
      const finalPrice = resolveFinalPrice(l);
      const lineFee    = base > 0 && l.feePayer === "buyer"
  ...
  // Label legível da taxa: 0.07 → "7%", fee.fixed → "+ R$ 0,50" se houver
  const feeLabel = [
        ...
        <div className="sticky top-24 bg-white border-2 border-black rounded-[24px] p-6 md:p-8 shadow-[6px_6px_0px_#0A0A0A] flex flex-col gap-6">
                ...
                <div className="text-left">
                  ...
                  <p className="font-body text-[11px] text-gray-500 mt-0.5">
                    ...
                    <span className="font-bold text-black">
                      {formatPrice(marketMin).free ? "Gratuito" : formatBRL(marketMin)}
                    </span>
                  </p>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-3">
            {officialLots.length === 0 ? (
              ...
              officialLots.map(lot => {
                const qty          = qtys[lot.id] || 0;
                const active       = qty > 0;
                const isAvailable  = lot.available ?? true;
                const blocked      = isAvailable && isLotBlocked(lot);
                const reasonLabel  = !isAvailable && lot.unavailableReason
                  ? UNAVAILABLE_LABEL[lot.unavailableReason] ?? null
                  : null;
                const displayPrice = safeNum(lot.price); // preço base, sem taxa
                const { label: priceLabel, free } = formatPrice(displayPrice);
                const hasFee = !free && lot.feePayer === "buyer" && (safeNum(lot.feePercentage) + fee.fixed) > 0;
                const lotFeeLabel = lot.feePercentage > 0 ? formatPercent(safeNum(lot.feePercentage)) : null;
                const lotIsFree = safeNum(lot.price) === 0;
                  ...
                  >
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex flex-col gap-1.5">
                        <h3 className={`font-bricolage font-extrabold text-[16px] uppercase leading-tight ${
                          !isAvailable || blocked ? "text-gray-400" : "text-black"
                        }`}>
                          {lot.title}
                        </h3>
                        {lot.subtitle && (
                          <p className="font-body text-[12px] text-gray-500">{lot.subtitle}</p>
                        )}
                        {reasonLabel && (
                        ...
                        {blocked && (
                          <span className="self-start text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-gray-100 text-gray-400">
                            {lotIsFree ? "Incompatível com pago" : "Incompatível com gratuito"}
                          </span>
                        )}
                      </div>
                      <div className="text-right shrink-0">
                        <span className={`font-bricolage font-extrabold text-xl leading-none ${
                          !isAvailable || blocked ? "text-gray-400" : free ? "text-primary" : "text-black"
                        }`}>
                          {priceLabel}
                        </span>
                        {hasFee && lotFeeLabel && isAvailable && !blocked && (
                          <p className="font-body text-[10px] text-gray-400 mt-0.5">inclui taxa {lotFeeLabel}</p>
                        )}
                      </div>
                    </div>
                    ...
                    {isAvailable && !blocked && (
                      <>
                        ...
                        <div className="flex items-center justify-between">
                          <span className="font-body text-[11px] font-bold uppercase tracking-widest text-gray-400">Qtd.</span>
                          <div className="flex items-center gap-3 bg-gray-100 rounded-full px-1 py-1">
                            <button
                              ...
                              className={`w-8 h-8 rounded-full flex items-center justify-center text-lg font-bold transition-all ${
                                qty > 0 ? "bg-black text-white hover:bg-black/80" : "text-gray-300 cursor-not-allowed"
                              }`}
                            >−</button>
                            <span className="font-body font-extrabold text-black w-4 text-center text-sm">{qty}</span>
                            <button
                              onClick={() => inc(lot.id)}
          ...
          <div className="flex flex-col gap-4">
            {totalQty > 0 && (
              <>
                ...
                <div className="flex items-baseline justify-between">
                  ...
                  <span
                    className={`font-bricolage font-extrabold text-[28px] leading-none ${allFree ? "text-primary" : "text-black"}`}
            ...
            >
              {totalQty > 0
                ? allFree ? "Garantir ingresso" : "Comprar agora"
