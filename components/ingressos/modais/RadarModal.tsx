...
import { X, Lightning, InstagramLogo, EyeSlash, Eye, LockKey, Info, SpinnerGap, ArrowLeft } from '@phosphor-icons/react'
...
export default function RadarModal({ ingresso, onClose }: RadarModalProps) {
  ...
  // ── Alterna viewMode e carrega bloqueados sob demanda ──
  ...
  const handleUnblock = useCallback(async (userId: string) => {
    ...
    try {
      ...
      // Recarrega perfis para o usuário desbloqueado reaparecer no radar
        ...
        <div className="bg-white p-6 border-b border-[#E0E0D8] shrink-0">
          <div className="flex items-center justify-between w-full mb-4">
            <div className="flex items-center gap-2">
              {/* Botão voltar quando está na view de bloqueados */}
              {viewMode === 'blocked' && (
                <button
              ...
              <div>
                <h3 className="font-bricolage text-[24px] font-extrabold text-[#0A0A0A] tracking-tighter leading-none">
                  {viewMode === 'blocked' ? 'Bloqueados' : 'Reppy Radar'}
                </h3>
            ...
            <div className="flex items-center gap-2">
              {/* Botão de acesso à view de bloqueados */}
              {viewMode === 'radar' && (
                <button
                  onClick={() => handleSetViewMode('blocked')}
                  className="w-8 h-8 shrink-0 rounded-full bg-[#F0F0EB] flex items-center justify-center hover:bg-[#E0E0D8] transition-colors"
          ...
          {/* Conteúdo do header condicional por viewMode */}
          {viewMode === 'radar' && (
            <>
              {/* Taps mútuos */}
              {tapsRecebidos > 0 && (
                ...
                <div className="flex flex-col gap-2">
                  {blockedUsers.map(user => {
                    ...
                    return (
                      <div
                        key={user.userId}
                        className="bg-white rounded-[16px] px-4 py-3 flex items-center gap-3 border border-[#E0E0D8]"
                      >
                        {/* Avatar */}
                  ...
                  <p className="text-center font-body text-[12px] text-[#9A9A8F] mt-4 mb-2">
                    {blockedUsers.length} usuário{blockedUsers.length > 1 ? 's' : ''} bloqueado{blockedUsers.length > 1 ? 's' : ''}.
                  </p>
                </div>
              )}
            </>
