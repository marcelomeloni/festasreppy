...
export default function TicketCard({ ingresso }) {
  ...
  return (
    <>
      <div className={`bg-white rounded-[20px] border-[1.5px] border-[#E0E0D8] overflow-hidden ${isDone ? 'opacity-55' : ''}`}>
        ...
        {/* ── Badge: anunciado no Market (clicável para editar) ── */}
          ...
          >
            ...
            <span className="font-body text-[12px] text-[#5C5C52] flex-1">
              ...
              <span className="font-bold text-black">
                {ingresso.listingPrice.toLocaleString('pt-BR', {
                  style: 'currency',
                  currency: 'BRL',
                })}
              </span>
      ...
      {showQR && (
        <QRModal ingresso={ingresso} onClose={() => setShowQR(false)} />
      )}

      {showRadar && (
        <RadarModal ingresso={ingresso} onClose={() => setShowRadar(false)} />
      )}
      ...
      {showTransfer && (
        <TransferModal ingresso={ingresso} onClose={() => setShowTransfer(false)} />
      )}

      {showSell && (
        <SellModal ingresso={ingresso} onClose={() => setShowSell(false)} />
      )}

      {showRefund && (
        <RefundModal ingresso={ingresso} onClose={() => setShowRefund(false)} />
      )}
    </>
  )
}